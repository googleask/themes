...
function footer_news_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
    ...
    if($tmp['article']) {
        ...
        foreach ($tmp['article'] as $idx=>$row) {
            ...
            if($limit>0){
                if($idx == 0){
                    $html[$key] = "<dd class='article-index-list'><ul>".footer_news_html($tmp_lv, $url, $article_title,'article-index');
                }elseif($idx == ($len-1)){
                    $html[$key] = footer_news_html($tmp_lv, $url, $article_title,'article-index')."</ul></dd>";
                }else{
...
function footer_news_html($lv, $url, $name,$type) {
    $tag = $lv == '1' ? 'dd' : 'li';
    return <<<EOF
<{$tag} class="{$type} lv-{$lv} content-item">
